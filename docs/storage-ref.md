...
Until Phase 17c, `storage_provider` and `storage_ref` may be set manually via PATCH but do not change where keyring bytes are stored. When both fields are set, the API validates `storage_ref` against this URI contract via `StorageRefParser`. Phase 17b adds **Test connection** (STS AssumeRole + S3 probe) without changing keyring storage location.
