...
- **Phase 18 (implemented):** SSH setup pack for authenticate subkeys — `POST /api/keys/{keyId}/export-ssh-private` (OpenSSH PEM) and `POST /api/keys/{keyId}/export-ssh-setup-pack` (AES-256 zip via zip4j with one-time `X-Archive-Password`); Overview **SSH setup** card (`SshSetupCard`) with server `.pub` export + pack download; blocking one-time password dialog; OpenSSH private key inside the zip is unencrypted; password never logged or stored in the zip; `[pgp-ui]` `keyDetail.sshSetup.*`; backend ops `export_ssh_private` / `export_ssh_setup_pack`.
