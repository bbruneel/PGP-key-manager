...
- **Supabase Postgres (Architecture 1):** Spring connects with JDBC; schema is applied by **Flyway** from `backend/src/main/resources/db/migration/`. Use the pooler URL for the app datasource and the direct URL for Flyway (see the example secret file). Cloud project: `pgp-key-manager` (`vwjmyednpakdcunrtyog`). The transaction pooler (port **6543**) does not support JDBC server-side prepared statements — include `prepareThreshold=0` in the pooler JDBC URL (the example secret file does). The app also sets this automatically when it detects a pooler URL.
